...
# the results showed low correlation between the predictos and hence
# a low chance of issues with multicollinearity
```
```{r}
# the QQ plot falls well along the 45 degree line of the plot and indicates that the normal
# distribution is conserved here. The residuals vs. fitted values show a bit of a funnel but 
# for the most part result in a fairly straight line which indicates good fit
par(mfrow = c(2, 2))
plot(fit.1)
```
# residual analysis
